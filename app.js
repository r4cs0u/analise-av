(() => {
  ...
  const SIGNAL_PROFILES = {
    sdr709: {
      label: 'SDR BT.709',
      desc:  'HD SDR \u00b7 Escala 0\u2013100 nit \u00b7 Gamut BT.709',
      nitsMax: 100, nitsLog: false,
      nitsRefs: [
        { v:100, l:'100 nit', c:'rgba(255,107,129,.7)' },
        { v:75,  l:'75',     c:'rgba(153,163,173,.4)' },
        { v:50,  l:'50',     c:'rgba(153,163,173,.4)' },
        { v:25,  l:'25',     c:'rgba(153,163,173,.25)' },
        { v:0,   l:'0',      c:'rgba(153,163,173,.25)' },
      ],
      vsStd: 'bt709',
      histLegal: { lo:16, hi:235 },
    },
    hlg2020: {
      label: 'HLG BT.2020',
      desc:  '4K/HD HDR HLG \u00b7 Escala 0\u20131000 nit \u00b7 Gamut BT.2020',
      nitsMax: 1000, nitsLog: false,
      nitsRefs: [
        { v:1000, l:'1000 nit', c:'rgba(255,107,129,.7)' },
        { v:203,  l:'203 ref',  c:'rgba(119,178,85,.7)' },
        { v:100,  l:'100',     c:'rgba(153,163,173,.35)' },
        { v:0,    l:'0',       c:'rgba(153,163,173,.25)' },
      ],
      vsStd: 'bt2020',
      histLegal: { lo:16, hi:235 },
    ...
    pq2020: {
      label: 'PQ BT.2020',
      desc:  '4K/HD HDR PQ \u00b7 Escala 0\u201310000 nit (log) \u00b7 Gamut BT.2020',
      nitsMax: 10000, nitsLog: true,
      nitsRefs: [
        { v:10000, l:'10k nit', c:'rgba(255,107,129,.7)' },
        { v:4000,  l:'4000',   c:'rgba(255,180,84,.55)' },
        { v:1000,  l:'1000',   c:'rgba(255,180,84,.45)' },
        { v:203,   l:'203 ref',c:'rgba(119,178,85,.7)' },
        { v:100,   l:'100',    c:'rgba(153,163,173,.35)' },
        { v:0,     l:'0',      c:'rgba(153,163,173,.25)' },
      ],
  ...
  const state = {
    ...
    modules: {
      ...
      spectrum: true, phase: true, loudness: true, falsecolor: false
    },
  ...
  const el = {
    ...
    falseColorCanvas: $('falseColorCanvas'),
  ...
  function initCanvases() {
    ...
    // false color canvas acompanha video
    const fc = el.falseColorCanvas;
    if (fc) { fc.width = el.video.offsetWidth || 640; fc.height = el.video.offsetHeight || 360; }
  }
  ...
  function drawWaveformGrid(ctx, w, h) {
    ...
    if (state.wfNits) {
      // Grade baseada nos nitsRefs do perfil
      const refs = prof.nitsRefs;
      const nMax = prof.nitsMax;
      font(ctx,10); ctx.textBaseline='middle';
      refs.forEach((ref, i) => {
        const yn = prof.nitsLog
          ? (ref.v > 0 ? Math.log10(ref.v) / Math.log10(nMax) : 0)
          : ref.v / nMax;
        const y = h - yn * h;
  ...
  function drawWaveform(pts, w, h) {
    ...
    const modeLabel = {luma:'Luma',rgb:'RGB Parade',ycbcr:'YCbCr'}[mode]||mode;
    ctx.fillText((state.wfNits ? getProfile().label+' Nits \u00b7 ' : '') + modeLabel, w-4, 4);
  }
  ...
  function drawVectorscopeBg(ctx, cx, cy, radius) {
    ...
    // marcadores de gamut
    const std = state.vsStd;
    VS_COLORS.forEach(({label,r,g,b}) => {
      const {cb,cr} = rgbToVS(r,g,b,std);
  ...
  function drawHistogram(histR, histG, histB, w, h) {
    ...
    // ── zona de clipping (legal range highlight) ────────────────────────────
    if (state.histRange === 'legal') {
      const xLo = (legal.lo/255)*w;
      const xHi = (legal.hi/255)*w;
    ...
    for (let i=0; i<256; i++) {
      const x  = i*bw;
      const isClipLo = i <= legal.lo;
      const isClipHi = i >= legal.hi;
      const clipTint = state.histRange==='legal' && (isClipLo || isClipHi);
      const alphaR = clipTint ? '.75' : '.42';
      const alphaG = clipTint ? '.75' : '.42';
      const alphaB = clipTint ? '.75' : '.42';
      ctx.fillStyle=`rgba(255,80,100,${alphaR})`;
      ctx.fillRect(x, h-14-(histR[i]/mx)*(h-14), Math.max(1,bw), (histR[i]/mx)*(h-14));
      ctx.fillStyle=`rgba(80,200,80,${alphaG})`;
      ctx.fillRect(x, h-14-(histG[i]/mx)*(h-14), Math.max(1,bw), (histG[i]/mx)*(h-14));
      ctx.fillStyle=`rgba(80,140,255,${alphaB})`;
      ctx.fillRect(x, h-14-(histB[i]/mx)*(h-14), Math.max(1,bw), (histB[i]/mx)*(h-14));
    }
    // ── r\u00f3tulo clipping bins 0 e 255 ────────────────────────────────────────
    if (state.histRange === 'legal') {
      const clipR0 = histR[0]+histR[1]+histR[2]; // near-black clipping
      const clipR255 = histR[253]+histR[254]+histR[255]; // near-white clipping
      font(ctx,9);
      if (clipR0 > 0) {
        ctx.fillStyle='rgba(255,180,84,.9)'; ctx.textAlign='left'; ctx.textBaseline='top';
        ctx.fillText('\u25bc clip', 2, 2);
      }
      if (clipR255 > 0) {
        ctx.fillStyle='rgba(255,107,129,.9)'; ctx.textAlign='right'; ctx.textBaseline='top';
        ctx.fillText('clip \u25bc', w-2, 2);
      }
    }
  }
  ...
  function renderFalseColor() {
    if (!state.modules.falsecolor || !el.video.videoWidth) return;
    const fc = el.falseColorCanvas;
    const vw = el.video.offsetWidth  || el.video.videoWidth  || 640;
    const vh = el.video.offsetHeight || el.video.videoHeight || 360;
    if (fc.width !== vw || fc.height !== vh) { fc.width=vw; fc.height=vh; }
    const ctx = fc.getContext('2d');
    // desenha frame no offscreen em tamanho reduzido para performance
    const sw = Math.round(vw / 2), sh = Math.round(vh / 2);
    offscreen.width=sw; offscreen.height=sh;
    offctx.drawImage(el.video, 0, 0, sw, sh);
    let imgData;
    try { imgData = offctx.getImageData(0, 0, sw, sh); } catch(e) { return; }
    const d = imgData.data;
    const out = ctx.createImageData(vw, vh);
    const od  = out.data;
    // false color LUT baseada em luma
    for (let py=0; py<vh; py++) {
      for (let px=0; px<vw; px++) {
        const spx = Math.floor(px * sw / vw);
        const spy = Math.floor(py * sh / vh);
        const si  = (spy*sw+spx)*4;
        const luma = 0.2126*d[si]+0.7152*d[si+1]+0.0722*d[si+2];
        const ln   = luma/255;
        let r=0,g=0,b=0,a=200;
        if      (ln < 0.04) { r=0;  g=0;  b=180; }  // blue: near-black
        else if (ln < 0.12) { r=0;  g=80; b=255; }  // cyan-blue
        else if (ln < 0.35) { r=0;  g=180;b=0;   }  // green: mid-low
        else if (ln < 0.60) { r=0;  g=0;  b=0; a=0; } // transparent: mid
        else if (ln < 0.75) { r=220;g=180;b=0;   }  // yellow: mid-high
        else if (ln < 0.90) { r=255;g=80; b=0;   }  // orange
        else                { r=255;g=0;  b=0;   }  // red: near-white
        const di = (py*vw+px)*4;
  ...
  function analysisLoop(ts) {
      ...
      try {
        ...
        state.worker.postMessage(
          { buf:imgData.data.buffer, fw, fh,
            ...
            vsStd:state.vsStd, wfMode:state.wfMode },
          [imgData.data.buffer]
        );
      } catch(e) {
    ...
    if (state.modules.falsecolor) renderFalseColor();
    renderAudioSpectrum();
    renderPhase();
    renderLoudness();
  ...
  function updateHlsMetadata() {
    ...
    if (maxLvl) {
      ...
      el.metaGamma.textContent=getProfile().label+' / '+vr;
    }
  ...
  function updateVideoMetadata() {
    ...
    if (!state.hls) {
      ...
      el.metaGamma.textContent=getProfile().label+' (prov\u00e1vel)';
  ...
  function populateHlsQuality() {
    ...
    state.hls.levels.forEach((lvl,i) => {
      ...
      opt.value=String(i);
      // item 5: thumbnail com resolucao + bitrate + fps
  ...
  function loadFromUrl() {
    ...
    if (window.Hls && Hls.isSupported() && (url.includes('.m3u8') || url.includes('m3u8'))) {
  ...
  function initProfileButtons() {
    el.profileGroup.querySelectorAll('.profile-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        state.signalProfile = btn.dataset.profile;
        const prof = getProfile();
        el.profileDesc.textContent = prof.desc;
        // sincroniza vsStd e vsStdGroup com o perfil
        state.vsStd = prof.vsStd;
        el.vsStdGroup.querySelectorAll('.scope-btn').forEach(b => {
          b.classList.toggle('active', b.dataset.vsstd===prof.vsStd);
        });
        updateVideoMetadata();
      });
    });
  ...
  function initModuleToggles() {
    ...
    Object.entries(map).forEach(([id,key]) => {
      ...
      inp.addEventListener('change', () => {
        ...
        if (key==='falsecolor') {
          el.falseColorCanvas.classList.toggle('active', inp.checked);
          if (!inp.checked) {
            const ctx=el.falseColorCanvas.getContext('2d');
            ctx.clearRect(0,0,el.falseColorCanvas.width,el.falseColorCanvas.height);
          }
          return;
        }
